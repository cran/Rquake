...
#################   rectify all date/times to same minute mark
    ...
    d1 = recdate(jd=g1$jd, hr=g1$hr, mi=g1$mi, sec=g1$sec, yr=g1$yr)
    Adate = rangedatetime(d1)
    mdate = Adate$min
   YY = YRsecdif(mdate$jd, mdate$hr, mdate$mi, 0, d1$jd, d1$hr, d1$mi, d1$sec, yr1 = mdate$yr, yr2 = d1$yr)
   dm =  getmoday(mdate$jd, mdate$yr)
    ret1 = g1
    n = length(YY)
    ret1$sec = YY
